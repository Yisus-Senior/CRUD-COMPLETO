...
public class GUIConsultBalanceController {
	...
	public void consultStudentBalance(ActionEvent event) {
		if(validateCarnet()) { //valida si el carnet 
			if(LogicStudents.existCarnet(tfConsultBalance.getText())) { //metodo de logica que valida si el estudiante existe a partir del carnet
				List<StudentRecharge> recharges = LogicRecharges.getStudentRecharges(tfConsultBalance.getText());
				if(!recharges.isEmpty()) {// si lista de recargas no esta vacia
					//lo convierte en observable lista 
...
//					tvColumnBalance.setCellValueFactory(cellData -> cellData.getValue().amountProperty());
	...
	//valida si el carnet esta en blanco o si es mayor a 10 dijitos
